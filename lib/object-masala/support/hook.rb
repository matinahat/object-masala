...
module ObjectMasala
  module Hook
    ...
    module ClassMethods
      extend ObjectMasala::LocalObjectSpace
      include ObjectMasala::Assertions
      ...
      def inline_call(method_info, scope)
        ObjectMasala::Hook::ClassMethods.hook_scopes << method_info[:from]
        name = method_info[:name]
        if scope == :instance
          args = method_defined?(name) && instance_method(name).arity != 0 ? '*args' : ''
          %(#{name}(#{args}) if self.class <= ObjectMasala::Hook::ClassMethods.object_by_id(#{method_info[:from].object_id}))
        else
          args = respond_to?(name) && method(name).arity != 0 ? '*args' : ''
          %(#{name}(#{args}) if self <= ObjectMasala::Hook::ClassMethods.object_by_id(#{method_info[:from].object_id}))
        end
      end
